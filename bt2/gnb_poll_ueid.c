...
#define SOCKET int
#define GNB_BACKLOG 20
#define MAX_BUF 1024
#define PORT "5001" // Port to listen()
#define UE_LIST_SIZE 20
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        printf(GRN "Timer KPI: %ld.%06ld sec\n" RESET, t2.tv_sec - t1.tv_sec, t2.tv_usec - t1.tv_usec);
...
void msg_handle(unsigned char buffer[], SOCKET sender_fd, int *ue_count, int bytes_recv)
{
    ...
    else if (buffer[0] == 3)
    {
        if (check == -1)
        {
            ...
            check = -2; // Dont calculate time anymore
        }
